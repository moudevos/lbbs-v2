...
export async function GET() {
  ...
  if (error) {
    console.error("[service-categories/get] Error al listar categorias", {
      message: error.message,
      code: error.code,
    });
    return NextResponse.json(
      { error: "No se pudieron cargar las categorias de servicios." },
      { status: 500 },
    );
  }
...
export async function POST(request: Request) {
  ...
  if (error) {
    console.error("[service-categories/post] Error al crear categoria", {
      message: error.message,
      code: error.code,
    });
    return NextResponse.json(
      {
        error: toFriendlyDatabaseError(
          error,
          "No se pudo crear la categoria de servicios.",
        ),
      },
      { status: 400 },
    );
