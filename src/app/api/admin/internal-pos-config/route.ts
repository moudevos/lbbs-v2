...
export async function GET() {
  ...
  const [employees, customers, links, rules, branches, services, products] = await Promise.all([
    ...
    supabase.from("employee_benefit_rules").select("*").order("created_at", { ascending: false }),
    supabase.from("branches").select("id,name").eq("is_active", true).order("name"),
    supabase.from("services").select("id,name").eq("is_active", true).order("name"),
    supabase.from("products").select("id,name").eq("is_active", true).order("name"),
  ]);
  const error = employees.error ?? customers.error ?? links.error ?? rules.error ?? branches.error ?? services.error ?? products.error;
  if (error) return NextResponse.json({ error: "No se pudo cargar la configuración interna." }, { status: 500 });
  return NextResponse.json({ data: { employees: employees.data ?? [], customers: customers.data ?? [], links: links.data ?? [], rules: rules.data ?? [], branches: branches.data ?? [], services: services.data ?? [], products: products.data ?? [] } });
}
...
export async function POST(request: Request) {
  ...
  if (body?.action === "link") {
    ...
    const query = body.id ? supabase.from("employee_benefit_rules").update({ ...values, updated_at: new Date().toISOString() }).eq("id", body.id) : supabase.from("employee_benefit_rules").insert(values);
