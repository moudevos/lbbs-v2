...
export async function POST(request:Request,context:{params:Promise<{kind:string}>}){
  ...
  const payload=await request.json().catch(()=>null);if(!payload?.name)return NextResponse.json({error:"El nombre es obligatorio."},{status:400});if(rawKind==="product_bonus"&&(!payload.scope_id||!["product","product_category","service","service_category"].includes(String(payload.scope_type))))return NextResponse.json({error:"Selecciona el producto, servicio o categoria al que aplica el bono."},{status:400});const supabase=await createClient();const [{data:employeeId},{data:businessDate,error:businessDateError}]=await Promise.all([supabase.rpc("current_employee_id"),supabase.rpc("pos_business_date")]);if(businessDateError||!businessDate)return NextResponse.json({error:"No se pudo determinar la fecha operativa."},{status:500});const values=buildValues(rawKind,payload,employeeId??null,businessDate);if(payload.id)delete (values as Partial<typeof values>).created_by;
