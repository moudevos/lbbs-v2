...
function buildValues(kind: RuleKind, payload: Record<string, unknown>, employeeId: string | null) {
  ...
  if (kind === "product_bonus") return { ...base, product_id: payload.scope_type === "product" ? payload.scope_id || null : null, product_category_id: payload.scope_type === "category" ? payload.scope_id || null : null, bonus_type: "fixed_per_unit", bonus_value: Number(payload.value) };
...
export async function POST(request:Request,context:{params:Promise<{kind:string}>}){
  ...
  const payload=await request.json().catch(()=>null);if(!payload?.name)return NextResponse.json({error:"El nombre es obligatorio."},{status:400});const supabase=await createClient();const {data:employeeId}=await supabase.rpc("current_employee_id");const values=buildValues(rawKind,payload,employeeId??null);if(payload.id)delete (values as Partial<typeof values>).created_by;
