...
export async function GET(request: Request) {
  ...
  if (error) {
    console.error("[product-branch-prices/get] Error al listar precios por sede", {
  ...
  return NextResponse.json({
    data: (data ?? []).map((row) => formatBranchPrice(row as BranchPriceRow)),
  });
}
...
export async function POST(request: Request) {
  ...
  if (error) {
    console.error("[product-branch-prices/post] Error al crear precio especial", {
      message: error.message,
      code: error.code,
      productId,
      branchId,
