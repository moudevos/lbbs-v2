...
declare
  ...
  v_authorization_reason text := nullif(btrim(coalesce(p_payload ->> 'authorization_reason', '')), '');
...
begin
  ...
  if v_rule_id is not null then
    ...
    if v_rule.eligible_role is not null and v_rule.eligible_role <> v_linked_employee.role then
    ...
    if v_rule.is_internal_complimentary then
      if public.current_user_role() <> 'owner' or v_authorization_reason is null then
        raise exception 'El consumo sin cobro requiere owner activo y motivo obligatorio.';
      end if;
  ...
  if v_operation_kind <> 'customer' then
    insert into public.internal_pos_operations (
      ...
      v_authorization_reason,
      case when v_operation_kind = 'internal_complimentary' then v_employee_id else null end,
      v_employee_id
    );
  end if;
