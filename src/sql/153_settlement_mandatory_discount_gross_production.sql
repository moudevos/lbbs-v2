-- Ejecutar después de 152_operational_contribution_per_service_unit.sql.
-- El 1 % obligatorio se descuenta de la producción/ganancia bruta de la
-- liquidación, no del saldo que queda luego de deudas o ajustes.
-- También limita el alcance POS de un admin que tiene sede asignada.
...
as $$
  select coalesce(
    ...
    or (
      ...
      and (public.current_branch_id() is null or public.current_branch_id() = target_branch_id)
    )
    or (
...
begin
  ...
  new.mandatory_discount_amount := round(greatest(coalesce(new.gross_pay_amount, 0), 0) * new.mandatory_discount_rate / 100, 2);
...
begin
  if not (public.is_owner() or public.is_admin()) then raise exception 'Solo owner o admin pueden revisar liquidaciones.'; end if;
  select * into v_settlement from public.employee_settlements where id = p_settlement_id for update;
  if not found or v_settlement.status <> 'draft' then raise exception 'Solo una liquidacion en borrador puede revisarse.'; end if;
  ...
  for v_adjustment in select * from jsonb_array_elements(coalesce(p_adjustments, '[]'::jsonb)) loop
    ...
    if v_type not in ('bonus', 'deduction') or v_description is null or v_amount <= 0 then raise exception 'Cada ajuste necesita tipo, motivo y monto mayor a cero.'; end if;
  ...
  v_mandatory := round(greatest(coalesce(v_settlement.gross_pay_amount, 0), 0) * coalesce(v_settlement.mandatory_discount_rate, 1) / 100, 2);
  ...
  set manual_bonus_total = v_bonus,
      ...
      status = 'review', reviewed_by = v_employee, reviewed_at = now()
