...
type Option = { id: string; name?: string; full_name?: string; start_date?: string; end_date?: string };
type Relation = { name?: string; full_name?: string } | Array<{ name?: string; full_name?: string }> | null;
type ProductionRow = {
  id: string; employee_id: string; branch_id: string; sale_id: string; production_date: string;
  production_source: "normal" | "reward" | "courtesy" | "commercial_discount";
  quantity: number | string; original_line_total: number | string; commercial_discount_amount: number | string;
  reward_discount_amount: number | string; courtesy_discount_amount: number | string; collected_amount: number | string;
  operational_contribution_amount: number | string; commissionable_amount: number | string; fixed_commission_amount: number | string;
  status: "active" | "reversed"; reversed_at?: string | null; reversed_reason?: string | null; employee: Relation; branch: Relation; service: Relation; sale?: { status?: string; cancelled_at?: string | null; cancelled_reason?: string | null } | Array<{ status?: string; cancelled_at?: string | null; cancelled_reason?: string | null }> | null;
};
type BonusRow = {
  id: string; employee_id: string | null; branch_id: string; sale_id: string; quantity: number | string;
...
const sourceLabels = { normal: "Normal", reward: "Reward", courtesy: "Cortesia", commercial_discount: "Descuento comercial" };
const money = (value: number | string | null | undefined) => Number(value ?? 0);
const relationLabel = (value: Relation, key: "name" | "full_name") => (Array.isArray(value) ? value[0]?.[key] : value?.[key]) ?? "Sin registro";
...
export function ProductionPageClient() {
  ...
  const [productionStatus, setProductionStatus] = useState<"active" | "reversed" | "all">("active");
  const [isLoading, setIsLoading] = useState(true);
  const [isGenerating, setIsGenerating] = useState(false);
  const [detailEmployeeId, setDetailEmployeeId] = useState<string | null>(null);

  const loadData = useCallback(async (nextPeriodId = periodId) => {
    setIsLoading(true);
    try {
      const params = new URLSearchParams({ periodId: nextPeriodId, branchId, employeeId, source });
      const response = await fetch(`/api/admin/production?${params}`, { cache: "no-store" });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error || "No se pudo cargar la produccion.");
      setRows(payload.data ?? []); setBonuses(payload.bonuses ?? []); setDebts(payload.debts ?? []); setSettlements(payload.settlements ?? []);
      setPeriods(payload.filters?.periods ?? []); setBranches(payload.filters?.branches ?? []); setEmployees(payload.filters?.employees ?? []);
      if (!nextPeriodId && payload.selectedPeriodId) setPeriodId(payload.selectedPeriodId);
    } catch (error) {
      const message = error instanceof Error ? error.message : "No se pudo cargar la produccion.";
      console.error("[production/ui] Error al cargar", { message });
      await Swal.fire({ icon: "error", title: "No se pudo cargar la produccion", text: message, confirmButtonColor: "#0f766e" });
  ...
  const activeBonuses = bonuses.filter((row) => row.status === "active" || row.status === "pending_review");
  const summary = useMemo(() => ({
    services: activeRows.reduce((sum, row) => sum + money(row.quantity), 0),
    serviceGross: activeRows.reduce((sum, row) => sum + money(row.original_line_total), 0),
    contribution: activeRows.reduce((sum, row) => sum + money(row.operational_contribution_amount), 0),
    base: activeRows.reduce((sum, row) => sum + money(row.commissionable_amount), 0),
    products: activeBonuses.reduce((sum, row) => sum + money(row.quantity), 0),
    productGross: activeBonuses.reduce((sum, row) => sum + money(single(row.sale_item)?.total), 0),
    bonuses: activeBonuses.reduce((sum, row) => sum + money(row.total_bonus_amount), 0),
    rewards: activeRows.filter((row) => row.production_source === "reward").reduce((sum, row) => sum + money(row.quantity), 0),
    rewardFixed: activeRows.filter((row) => row.production_source === "reward").reduce((sum, row) => sum + money(row.fixed_commission_amount), 0),
    debts: debts.reduce((sum, debt) => sum + money(debt.outstanding_amount), 0),
  ...
  const employeeSummaries = useMemo(() => {
    ...
    for (const bonus of activeBonuses) if (bonus.employee_id) {
  ...
  async function generateProduction() {
    if (!periodId) return;
    const confirmation = await Swal.fire({ icon: "question", title: "Generar produccion del periodo", text: "Se procesaran solo ventas faltantes o que requieran reversion.", showCancelButton: true, confirmButtonText: "Generar", cancelButtonText: "Cancelar", confirmButtonColor: "#0f766e" });
    if (!confirmation.isConfirmed) return;
    setIsGenerating(true);
    try {
      const response = await fetch("/api/admin/production", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ periodId, branchId: branchId || null }) });
      const payload = await response.json(); if (!response.ok) throw new Error(payload.error);
      await loadData(periodId);
      await Swal.fire({ icon: "success", title: "Produccion actualizada", text: `Ventas revisadas: ${payload.data?.sales_reviewed ?? 0}. Servicios: ${payload.data?.services_generated ?? 0}. Bonos: ${payload.data?.bonuses_generated ?? 0}.`, confirmButtonColor: "#0f766e" });
    } catch (error) { const message = error instanceof Error ? error.message : "No se pudo generar."; console.error("[production/ui] Error al generar", { message }); await Swal.fire({ icon: "error", title: "No se pudo generar", text: message, confirmButtonColor: "#0f766e" }); } finally { setIsGenerating(false); }
  }
   ...
   <div className="space-y-4">
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <p className="shrink-0 text-sm font-semibold text-slate-900">Control de produccion</p>
        ...
        <div className="grid flex-1 gap-2 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <Select
            ...
            onChange={(event) =>
              setProductionStatus(event.target.value as "active" | "reversed" | "all")
            }
          >
    ...
    </section><section className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <table className="min-w-[1100px] w-full text-sm">
        <thead className="border-b border-slate-200 text-left text-xs uppercase tracking-wide text-slate-500">
          <tr>
            ...
            <th className="px-3 py-2.5 text-center font-semibold">Servicios</th>
            <th className="px-3 py-2.5 text-center font-semibold">Bruto servicios</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {isLoading ? (
            ...
            employeeSummaries.map((item) => {
              const settlementStyles: Record<string, { badge: string; dot: string }> = {
              ...
              const settlementStyle = item.settlement
                ? settlementStyles[item.settlement.status] ?? settlementStyles.draft
                : null;

              return (
                <tr key={item.id} className="transition hover:bg-slate-50">
                  <td className="px-3 py-2.5 font-medium text-slate-900">{item.name}</td>
                  <td className="px-3 py-2.5 text-slate-700">{item.branch}</td>
                  ...
                  <td className="px-3 py-2.5">
                    {item.settlement && settlementStyle ? (
                      ...
                      >
                        <span className={`h-1.5 w-1.5 rounded-full ${settlementStyle.dot}`} />
                        {getSettlementStatusLabel(item.settlement.status)}
                      </span>
                    ) : (
                      <span className="text-xs text-slate-400">Pendiente de porcentaje</span>
                    )}
                  </td>
