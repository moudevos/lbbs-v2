...
export function EmployeeSettlementPdf({ detail, services, bonuses: _bonuses, deductions }: Props) {
  ...
  return <Document title={`${title} ${String(detail.settlement_number ?? "")}`} author="La Bajadita Barber Studio"><Page size="A4" style={styles.page}>
    ...
    <View style={styles.section}><Text style={styles.heading}>Resumen de producción</Text><View style={[styles.box, styles.grid]}><View style={styles.summaryField}><Text style={styles.label}>Servicios</Text><Text style={styles.strong}>{summary.serviceCount}</Text></View><View style={styles.summaryField}><Text style={styles.label}>Productos</Text><Text style={styles.strong}>{summary.productCount}</Text></View><View style={styles.summaryField}><Text style={styles.label}>Rewards</Text><Text style={styles.strong}>{summary.rewardCount}</Text></View><View style={styles.summaryField}><Text style={styles.label}>Producción total</Text><Text style={styles.strong}>{amount(summary.totalProduction)}</Text></View><View style={styles.summaryField}><Text style={styles.label}>Aportes</Text><Text style={styles.strong}>-{amount(summary.productionDiscount)}</Text></View><View style={styles.summaryField}><Text style={styles.label}>Base comisionable</Text><Text style={styles.strong}>{amount(summary.productionBase)}</Text></View></View></View>
