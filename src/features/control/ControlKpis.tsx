...
type Payload = { financial: boolean; metrics: Record<string, number | null>; sessions: Array<{ id: string; branchName: string; status: string }> };
const money = (value: number | null) => new Intl.NumberFormat("es-PE", { style: "currency", currency: "PEN" }).format(value ?? 0);
...
export function ControlKpis() {
  ...
  return <div className="space-y-4"><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{cards.map(([label, value]) => <article key={label} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><p className="text-xs text-slate-500">{label}</p><p className="mt-2 text-lg font-semibold text-slate-900">{value}</p></article>)}</section><section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-sm font-semibold text-slate-900">Sesiones POS por sede</p><div className="mt-3 flex flex-wrap gap-2">{payload.sessions.length ? payload.sessions.map((session) => <span key={session.id} className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700">{session.branchName}: abierta</span>) : <p className="text-sm text-slate-500">No hay sesiones POS abiertas.</p>}</div></section></div>;
}
