...
type Payload = {
  ...
  branchReconciliations: Array<{ id: string; branchName: string; sessionStatus: string; grossSales: number; serviceGross: number; otherGross: number; rewardsCount: number; rewardsAmount: number; operationalContribution: number; closingTotal: number }>;
};
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 xl:grid-cols-3">
            {payload.branchReconciliations.map((branch) => {
              ...
              return <article key={branch.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-start justify-between gap-2"><p className="font-semibold text-slate-900">{branch.branchName}</p><span className={`rounded-full px-2 py-1 text-xs font-medium ${statusTone}`}>{status}</span></div><dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm"><div><dt className="text-xs text-slate-500">Bruto total</dt><dd className="font-semibold">{money(branch.grossSales)}</dd></div><div><dt className="text-xs text-slate-500">Cierre real</dt><dd className="font-semibold text-emerald-700">{money(branch.closingTotal)}</dd></div><div><dt className="text-xs text-slate-500">Servicios</dt><dd>{money(branch.serviceGross)}</dd></div><div><dt className="text-xs text-slate-500">Productos y otros</dt><dd>{money(branch.otherGross)}</dd></div><div><dt className="text-xs text-slate-500">Aportes producción</dt><dd>{money(branch.operationalContribution)}</dd></div><div><dt className="text-xs text-slate-500">Rewards cobrados</dt><dd>{branch.rewardsCount} · {money(branch.rewardsAmount)}</dd></div></dl></article>;
            })}
          </div>
        </section>
      ) : null}
