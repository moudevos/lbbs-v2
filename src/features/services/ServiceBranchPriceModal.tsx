...
type ServiceBranchPriceModalProps = {
  open: boolean;
  service: ServiceRecord | null;
  branches: BranchRecord[];
  prices: ServiceBranchPriceRecord[];
  value: ServiceBranchPriceFormValue;
  isSaving: boolean;
  onClose: () => void;
  onChange: (next: ServiceBranchPriceFormValue) => void;
  onSubmit: () => void;
  onToggleActive: (price: ServiceBranchPriceRecord) => void;
};
...
export function ServiceBranchPriceModal({
  open,
  service,
  branches,
  prices,
  ...
  return (
    <Modal
      ...
      description={`Gestiona el precio especial de ${service.name}.`}
      onClose={() => {
        if (!isSaving) {
          onClose();
        }
      ...
      <div className="space-y-5">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
          ...
          <p className="mt-3 text-sm text-slate-600">
            Precio base: <span className="font-semibold text-slate-900">{formatMoney(service.base_price)}</span>
          </p>
        </div>
              ...
              {prices.map((price) => (
                ...
                >
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">{formatMoney(price.price)}</p>
                  </div>
