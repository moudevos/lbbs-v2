...
begin
  ...
  if position(v_old in v_definition) = 0 then
    ...
    raise exception 'No se encontro la version esperada de checkout_pos_sale; ejecuta primero la migracion 135.';
  end if;
...
$old$,
    $new$    if v_rule_id is null and exists (
      select 1 from jsonb_array_elements(p_payload -> 'items') item
$new$
  );
  v_definition := replace(
    v_definition,
    $old$    v_operation_kind := 'employee_credit';
  end if;

  if v_operation_kind <> 'customer' and exists (
$old$,
    $new$    if v_operation_kind = 'customer' then
