-- Mirrors src/sql/139_configurable_product_courtesies.sql for Supabase CLI deploys.
...
begin
  ...
  select coalesce(sum(quantity * maximum_courtesy_items), 0),
         case when bool_or(maximum_courtesy_amount is null) then null else sum(quantity * maximum_courtesy_amount) end
  into v_total_capacity, v_total_amount_cap from matched_services;

  select coalesce(sum(quantity), 0), coalesce(sum(quantity * unit_price), 0)
  into v_capacity, v_total_amount
  from public.sale_items where sale_id = new.id and item_type = 'product' and is_courtesy;
  if v_total_capacity = 0 or v_capacity > v_total_capacity then raise exception 'La cantidad de productos en cortesía supera el cupo configurado para los servicios pagados.'; end if;
  ...
  for v_product_item in select * from public.sale_items where sale_id = new.id and item_type = 'product' and is_courtesy loop
    ...
    from matched_services ms join public.courtesy_rule_benefits benefit on benefit.rule_id = ms.rule_id and benefit.is_active
    where benefit.benefit_item_type = 'product' and benefit.product_id = v_product_item.product_id
      and (benefit.max_unit_amount is null or v_product_item.unit_price <= benefit.max_unit_amount)
    group by ms.rule_id, ms.rule_name order by sum(ms.quantity * benefit.max_quantity) desc limit 1;
...
create trigger sales_validate_configurable_courtesies after update of status on public.sales for each row execute function public.validate_completed_sale_courtesies();
...
begin
  ...
  if position(v_legacy_guard in v_definition) > 0 then execute replace(v_definition, v_legacy_guard, E'\n    -- Validado por sales_validate_configurable_courtesies.'); end if;
end;
$$;
