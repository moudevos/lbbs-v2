-- Mirrors src/sql/138_reception_production_and_employee_debts.sql.
...
begin
  ...
  if not (
    public.is_admin()
    or (public.current_user_role() = 'reception' and public.can_access_branch(p_branch_id))
  ) then
...
create policy "payroll_periods_reception_read" on public.payroll_periods for select to authenticated using (public.current_user_role() = 'reception');
drop policy if exists "employee_service_production_reception_scope" on public.employee_service_production;
create policy "employee_service_production_reception_scope" on public.employee_service_production for select to authenticated using (public.current_user_role() = 'reception' and public.can_access_branch(branch_id));
drop policy if exists "employee_product_bonus_entries_reception_scope" on public.employee_product_bonus_entries;
create policy "employee_product_bonus_entries_reception_scope" on public.employee_product_bonus_entries for select to authenticated using (public.current_user_role() = 'reception' and public.can_access_branch(branch_id));
drop policy if exists "employee_debts_reception_scope" on public.employee_debts;
create policy "employee_debts_reception_scope" on public.employee_debts for select to authenticated using (public.current_user_role() = 'reception' and public.can_access_branch(branch_id));
drop policy if exists "employee_debt_movements_reception_scope" on public.employee_debt_movements;
create policy "employee_debt_movements_reception_scope" on public.employee_debt_movements for select to authenticated using (public.current_user_role() = 'reception' and exists (select 1 from public.employee_debts debt where debt.id = employee_debt_movements.debt_id and public.can_access_branch(debt.branch_id)));
drop policy if exists "employee_settlements_reception_scope" on public.employee_settlements;
create policy "employee_settlements_reception_scope" on public.employee_settlements for select to authenticated using (public.current_user_role() = 'reception' and public.can_access_branch(branch_id));
